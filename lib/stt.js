...
import { modelSupportsAudio, sendAudioPassthrough, saveAudioToQueue } from "./passthrough.js";
  ...
  try {
    ...
    if (usePassthrough) {
      ...
      if (route?.name === "session" && route.params?.sessionID) {
        ...
        try {
          const tag = saveAudioToQueue(sessionID);
          await client.tui.appendPrompt({ text: tag + " " });
          toast(`Audio added as ${tag}`, "success");
          return;
        } catch (err) {
          toast(`Queue error: ${err.message}`, "error");
        }
      } else {
        toast("No active session to queue audio", "error");
        return;
      }
    }
