...
async function getAudioCapableModels(client, api) {
  ...
  // NOTE: We intentionally removed the call to client.config.providers() here.
  // The SDK implementation of config.providers() frequently throws "{} is not iterable"
  // internally in some environments/versions. Reading from opencode.json directly
  // is 100% reliable and contains the exact same schema data.
...
async function getActiveModel(client, api) {
  ...
  // Safe fallback to client.config.get()
  try {
