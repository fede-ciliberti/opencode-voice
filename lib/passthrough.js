...
async function getAudioCapableModels(client) {
  ...
  try {
    ...
    // Debug: log first provider's model structure to understand the schema
    if (providers.length > 0 && providers[0].models) {
      const firstProvider = providers[0];
      const firstModelKey = Object.keys(firstProvider.models)[0];
      const firstModelConfig = firstProvider.models[firstModelKey];
      const audioModelKeys = Object.entries(firstProvider.models)
        .filter(([, v]) => v.modalities?.input?.includes("audio"))
        .map(([k]) => k);
      log("provider_model_debug", {
        providerId: firstProvider.id,
        firstModelKey,
        hasModalities: !!firstModelConfig?.modalities,
        modalitiesValue: JSON.stringify(firstModelConfig?.modalities),
        modelConfigKeys: firstModelConfig ? Object.keys(firstModelConfig) : [],
        allModelCount: Object.keys(firstProvider.models).length,
        audioModelKeysFromThisProvider: audioModelKeys,
        hasGemini: Object.keys(firstProvider.models).some((k) => k.includes("gemini")),
      });
    }
