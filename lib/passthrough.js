...
async function getAudioCapableModels(client, api) {
  ...
  try {
    ...
    // Normalize providers to list (handles both array and object map formats)
    let providersList = [];
    if (result.data) {
      const rawProviders = result.data.providers || result.data;
      if (Array.isArray(rawProviders)) {
        providersList = rawProviders;
      } else if (typeof rawProviders === "object" && rawProviders !== null) {
        providersList = Object.values(rawProviders);
      }
    }
    ...
    for (const provider of providersList) {
      ...
      const modelsMap = typeof provider.models === "object" ? provider.models : {};
      for (const [modelKey, modelConfig] of Object.entries(modelsMap)) {
        if (!modelConfig || typeof modelConfig !== "object") continue;
        const inputModalities = modelConfig.modalities?.input || [];
        if (inputModalities.includes("audio")) {
          audioModels.add(modelKey);
